...
from research_os.docking.crossdock001_evaluator_v11 import run_corrected_crossdock001


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Execute frozen CROSSDOCK-001 docking with v1.1 pose representation evaluation"
    )
    parser.add_argument("--workdir", default=".crossdock001-run")
    parser.add_argument("--output", default=None)
    args = parser.parse_args()

    report = run_corrected_crossdock001(args.workdir)
    output = (
        Path(args.output)
        if args.output
        else Path(args.workdir) / "crossdock001-result-v1.1.json"
    )
    produced = Path(args.workdir) / "crossdock001-result-v1.1.json"
    ...
    if technical_failures:
        print(f"CROSSDOCK-001 v1.1 technical failures: {technical_failures}")
        return 1
