...
class KnowledgeLab(Lab):
    ...
    def normalize(self, raw: dict[str, Any]):
        ...
        return {
            ...
            "zettel_type": str(raw.get("zettel_type", "concept")).lower(),
            "domain": raw.get("domain"),
            "evidence_level": raw.get("evidence_level", EvidenceLevel.E0_HEURISTIC.value),
            "review_status": raw.get("review_status", ReviewStatus.REVIEW_REQUIRED.value),
    ...
    def rules(self):
        ...
        def sources(ctx, evidence):
            ...
            if ReviewStatus(ctx["review_status"]) == ReviewStatus.VERIFIED and not ss:
                return GateResult("GATE-KNOW-SOURCE", "KNOW-SRC-001", GateStatus.INSUFFICIENT_EVIDENCE, "a VERIFIED knowledge note must retain at least one source locator")
        ...
        rules += [
            Rule("KNOW-NOTE-002", "Validate Zettelkasten enum fields", enums),
            Rule("KNOW-SRC-001", "Require traceable sources for verified notes", sources),
            Rule("KNOW-REV-001", "Record review state without automatic promotion", lambda c, e: GateResult("GATE-KNOW-REVIEW", "KNOW-REV-001", GateStatus.PASS, "review state recorded explicitly")),
        ]
        return rules
    ...
    def run(self, raw: dict[str, Any], experiment: str = "zettel_ingestion"):
        ...
        z = Zettel(
            title=n["title"],
            summary=n["summary"],
            zettel_type=ZettelType(n["zettel_type"]),
            domain=n["domain"],
            evidence_level=EvidenceLevel(n["evidence_level"]),
            review_status=ReviewStatus(n["review_status"]),
            ...
            tags=n["tags"],
            links=n["links"],
            sources=tuple(SourceLocator(**s) for s in n["sources"]),
        )
        m.evidence.append(Evidence(evidence_id=f"EVD-{uuid.uuid4().hex[:12].upper()}", kind="zettelkasten_note", level=z.evidence_level, source="KnowledgeLab validated Zettel", payload={"zettel": z.to_dict(), "digest": z.digest}))
