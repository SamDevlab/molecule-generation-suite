...
class OracleWebApplication:
    ...
    def dispatch(self, method: str, path: str, body: dict[str, Any] | None = None) -> tuple[int, dict[str, Any]]:
        ...
        try:
            if method == "GET" and route == "/api/health":
                ...
                return 200, {"status": "ok", "service": "Research OS 4.0", "provider": dict(getattr(provider, "audit_metadata", {}) or {}), "oracle": self.oracle_status(), "ledger_source_of_truth": True}
    ...
    def serve(self, host: str = "127.0.0.1", port: int = 8000) -> None:
        ...
        print(f"Research OS 4.0 listening at http://{host}:{port}")
        try:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run the Research OS 4.0 operational Oracle app")
