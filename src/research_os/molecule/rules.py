...
def rdkit_structure_rule(
    rule_id: str = "MOL-STRUCT-002",
    calculator: RDKitCalculator | None = None,
) -> Rule:
    calc = calculator or RDKitCalculator()
    ...
    def evaluate(ctx: dict, evidence: list[Evidence]) -> GateResult:
        ...
        try:
            ...
            return GateResult(
                ...
                rule_id,
                GateStatus.FAIL,
                "RDKit could not parse/sanitize molecular representation",
                diagnostics={"error": str(exc)},
            )
        return GateResult(
            "GATE-STRUCTURE",
            rule_id,
            GateStatus.PASS,
            "molecular representation parsed and sanitized by RDKit",
            diagnostics={"canonical_smiles": props.canonical_smiles},
        )

    return Rule(
        rule_id,
        "Require a molecular representation that RDKit can parse and sanitize; this is not a physics/stability proof.",
        evaluate,
    )
