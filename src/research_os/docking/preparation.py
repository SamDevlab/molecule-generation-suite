...
class LigandPreparationRequest:
    ...
    seed: int | None = None
    options: tuple[str, ...] = ()
    hydrogen_treatment: str = "NOT_SPECIFIED"
    charge_method: str = "NOT_SPECIFIED"
...
class LigandPreparationManifest:
    ...
    def __post_init__(self) -> None:
        if self.manifest_hash is None:
            object.__setattr__(self, "manifest_hash", sha256_json({key: getattr(self, key) for key in ("candidate_id", "input_path", "output_path", "input_sha256", "output_sha256", "engine_id", "engine_version", "protocol_id", "status", "assumptions", "options", "hydrogen_treatment", "charge_method", "warnings")}))
...
def prepare_ligand(request: LigandPreparationRequest, engine: OpenBabelEngine | None = None, *, timeout: float = 60.0) -> LigandPreparationManifest:
    ...
    if not adapter.available or input_hash is None:
        return LigandPreparationManifest(request.candidate_id, request.input_path, request.output_path, input_hash, None, "openbabel", adapter.version, "openbabel.ligand-preparation.v1", "INDETERMINATE", request.protonation_assumptions, options=request.options, hydrogen_treatment=request.hydrogen_treatment, charge_method=request.charge_method, warnings=("Open Babel was unavailable or the source file was missing; no prepared artifact was produced.",))
    try:
        ...
        return LigandPreparationManifest(request.candidate_id, request.input_path, request.output_path, input_hash, None, "openbabel", adapter.version, "openbabel.ligand-preparation.v1", "INDETERMINATE", request.protonation_assumptions, options=request.options, hydrogen_treatment=request.hydrogen_treatment, charge_method=request.charge_method, warnings=("Open Babel conversion did not produce a verified artifact.",))
    output_hash = sha256_file(request.output_path) if Path(request.output_path).is_file() else None
    return LigandPreparationManifest(request.candidate_id, request.input_path, request.output_path, input_hash, output_hash, "openbabel", result.engine_version, result.protocol_id or "openbabel.ligand-preparation.v1", "SUPPORTED_AND_EXECUTED" if result.returncode == 0 and output_hash else "INDETERMINATE", request.protonation_assumptions, options=request.options, hydrogen_treatment=request.hydrogen_treatment, charge_method=request.charge_method, warnings=tuple(item for item in (result.stderr.strip(),) if item))
...
class ReceptorPreparationManifest:
    ...
    def __post_init__(self) -> None:
        if self.manifest_hash is None:
            object.__setattr__(self, "manifest_hash", sha256_json({key: getattr(self, key) for key in ("target_id", "species", "role", "structure_id", "source", "input_sha256", "prepared_sha256", "output_path", "engine_id", "engine_version", "protocol_id", "status", "metadata", "selected_chains", "retained_cofactors", "removed_components", "hydrogen_treatment", "charge_method", "raw_source_url", "raw_source_sha256", "options", "warnings")}))
    def to_dict(self) -> dict[str, Any]: return asdict(self)
...
def prepare_receptor(request: ReceptorPreparationRequest, engine: OpenBabelEngine | None = None, *, timeout: float = 120.0) -> ReceptorPreparationManifest:
    ...
    input_hash = sha256_file(source) if source.is_file() else None
    if not adapter.available or input_hash is None:
        return ReceptorPreparationManifest(request.target_id, request.species, request.role, request.structure_id, request.source, input_hash, None, request.output_path, "openbabel", adapter.version, "openbabel.receptor-preparation.v1", "INDETERMINATE", {"resolution_angstrom": request.resolution_angstrom, "method": request.method}, selected_chains=request.selected_chains, retained_cofactors=request.retained_cofactors, removed_components=request.removed_components, hydrogen_treatment=request.hydrogen_treatment, charge_method=request.charge_method, raw_source_url=request.raw_source_url, raw_source_sha256=request.raw_source_sha256, options=request.options, warnings=("Open Babel was unavailable or the source file was missing; no prepared artifact was produced.",))
    try:
        ...
        return ReceptorPreparationManifest(request.target_id, request.species, request.role, request.structure_id, request.source, input_hash, None, request.output_path, "openbabel", adapter.version, "openbabel.receptor-preparation.v1", "INDETERMINATE", {"resolution_angstrom": request.resolution_angstrom, "method": request.method}, selected_chains=request.selected_chains, retained_cofactors=request.retained_cofactors, removed_components=request.removed_components, hydrogen_treatment=request.hydrogen_treatment, charge_method=request.charge_method, raw_source_url=request.raw_source_url, raw_source_sha256=request.raw_source_sha256, options=request.options, warnings=("Open Babel conversion did not produce a verified artifact.",))
    output_hash = sha256_file(request.output_path) if Path(request.output_path).is_file() else None
    return ReceptorPreparationManifest(request.target_id, request.species, request.role, request.structure_id, request.source, input_hash, output_hash, request.output_path, "openbabel", result.engine_version, result.protocol_id or "openbabel.receptor-preparation.v1", "SUPPORTED_AND_EXECUTED" if result.returncode == 0 and output_hash else "INDETERMINATE", {"resolution_angstrom": request.resolution_angstrom, "method": request.method}, selected_chains=request.selected_chains, retained_cofactors=request.retained_cofactors, removed_components=request.removed_components, hydrogen_treatment=request.hydrogen_treatment, charge_method=request.charge_method, raw_source_url=request.raw_source_url, raw_source_sha256=request.raw_source_sha256, options=request.options, warnings=tuple(item for item in (result.stderr.strip(),) if item))
...
class ReceptorPreparationLab:
    ...
    def run(self, raw: dict[str, Any], experiment: str = "receptor_preparation") -> RunManifest:
        ...
        try:
            request = ReceptorPreparationRequest(target_id=str(raw["target_id"]), species=str(raw["species"]), role=str(raw["role"]), structure_id=str(raw["structure_id"]), source=str(raw["source"]), input_path=str(raw["input_path"]), output_path=str(raw["output_path"]), resolution_angstrom=raw.get("resolution_angstrom"), method=raw.get("method"), options=tuple(raw.get("options") or ()), selected_chains=tuple(raw.get("selected_chains") or ()), retained_cofactors=tuple(raw.get("retained_cofactors") or ()), removed_components=tuple(raw.get("removed_components") or ()), hydrogen_treatment=str(raw.get("hydrogen_treatment", "NOT_SPECIFIED")), charge_method=str(raw.get("charge_method", "NOT_SPECIFIED")), raw_source_url=raw.get("raw_source_url"), raw_source_sha256=raw.get("raw_source_sha256"))
...
def _preparation_run_result(manifest: RunManifest, result: Any, kind: str) -> RunManifest:
    if result.status != "SUPPORTED_AND_EXECUTED":
        manifest.gates.append(GateResult(f"GATE-{kind.upper()}", "OPENBABEL-PREP-001", GateStatus.INDETERMINATE, "Open Babel preparation was unavailable or did not produce a verified artifact", diagnostics={"status": result.status, "engine_id": result.engine_id})); return manifest.seal()
