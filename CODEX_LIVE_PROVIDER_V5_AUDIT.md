...
The repository now also provides a separate [top-level Live acceptance contract](TOP_LEVEL_LIVE_ACCEPTANCE_V5_0.md). It inspects process ancestry and environment before admission, fails closed in a Codex-owned task, and writes each bounded acceptance under a fresh `.research-os-live-5.0-top-level-attempt-N/` namespace. Attempts 1–8 are preserved; Attempt 8 is the completed 39/39 release acceptance and no additional Live run is required.

The first genuine top-level attempt is preserved at `.research-os-live-5.0-top-level/`: calls 1–16 completed under the Live boundary, and call 17 (`V5-FOLLOWUP-13`) completed with process/schema/cleanup `PASS` but was rejected by the previous boolean-only grounding gate. Attempt 2 is preserved separately at `.research-os-live-5.0-top-level-attempt-2/` and stopped at call 33 (`TL-CONSISTENCY-02-B`) when the real model returned the unregistered `CH-V45-SOLUBILITY-EXTERNAL-BOUNDARY`; the exact grounding and consistency failure remains visible. Attempts 3 and 4 are preserved in their numbered namespaces and stopped at call 30 (`TL-CONSISTENCY-01-A`) because the response omitted `primary_record_id` and `limitation_codes`; general grounding passed, Run B was not executed, and no scientific state changed. Attempt 4 proved that `live_output.schema.json` alone was enforced. Attempt 5 then proved the remaining provider propagation defect: the semantic consistency context was present only in the payload, so the transport correctly selected `ENVELOPE` for the context it received. Attempt 6 reached the corrected `CONSISTENCY` route but stopped at a provider process error before schema validation. The provider now reports `GroundingValidationResult`, `LiveResponseValidationFailure`, frozen consistency bases, canonical signatures, strict consistency contract diagnostics, fixed output-contract/schema metadata, allowlisted per-call context propagation with protected global owner/state fields, bounded typed schema-admission diagnostics, and final-gate failure precedence, while persisting only safe final structured fields from a rejected response. The provider-facing schema is limited to portable structural keywords; cross-field semantics remain deterministic validators.

## Release decision

The boundary hardening is testable and regression-safe. Attempt 7 exposed `PRIMARY_RECORD_DRIFT`; after the full consistency signature was frozen, Attempt 8 completed the real top-level Live response, three Live review roles, final Live exam, follow-ups, stress cases, and consistency checks at 39/39. The package is released as `5.0.0`. No additional Live acceptance or rerun is authorized.
