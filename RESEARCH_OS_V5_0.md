...
Status: **READY_FOR_TOP_LEVEL_SCHEMA_SMOKE**; release gate remains **BLOCKED_BEFORE_PASS**. Genuine top-level Attempt 1 completed 17/39 Live calls, Attempt 2 completed 33/39 before an unknown grounded ID was rejected, Attempts 3–4 completed 30/39 before the consistency response contract rejected missing `primary_record_id` and `limitation_codes`, Attempt 5 completed 30/39 before a per-call context propagation defect selected the outer envelope, and Attempt 6 completed 30/39 before a provider process error at the correctly routed consistency schema. The first real consistency-schema smoke then completed one `CONSISTENCY` call with provider, schema, grounding, consistency, and forbidden-field checks passing; its gate was blocked exclusively by the cleanup event aggregation bug, now corrected. Attempt 2 specifically exposed `MODEL_REFERENCE_HALLUCINATION` for the unregistered literal ID `CH-V45-SOLUBILITY-EXTERNAL-BOUNDARY`; the fail-closed validator was correct. The repository now binds the fixed six-field consistency schema to `codex exec`, propagates an allowlisted per-call semantic context through `CodexLiveProvider`, records the selected output contract, freezes Run A's support basis for Run B, and provides the next fresh smoke namespace before Attempt 7. No Live launcher is executed from the current Codex-owned task and no test provider is substituted.
...
The top-level acceptance contract, Attempts 1–6 records, grounding diagnostics, worktree policy, and official external command are documented in [TOP_LEVEL_LIVE_ACCEPTANCE_V5_0.md](TOP_LEVEL_LIVE_ACCEPTANCE_V5_0.md). Attempt 1 remains at `.research-os-live-5.0-top-level/`; Attempts 2–6 remain in their numbered namespaces; the next run first writes a fresh schema-smoke namespace and, only after a smoke `PASS`, writes `.research-os-live-5.0-top-level-attempt-7/` without overwriting prior attempts or recovery artifacts.
