...
The top-level acceptance contract, Attempts 1–4 records, grounding diagnostics, worktree policy, and official external command are documented in [TOP_LEVEL_LIVE_ACCEPTANCE_V5_0.md](TOP_LEVEL_LIVE_ACCEPTANCE_V5_0.md). Attempt 1 remains at `.research-os-live-5.0-top-level/`; Attempts 2, 3, and 4 remain in their numbered namespaces; the next run writes to `.research-os-live-5.0-top-level-attempt-5/` and does not overwrite prior attempts or recovery artifacts.
