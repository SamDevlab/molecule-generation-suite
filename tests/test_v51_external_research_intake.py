...
def test_external_intake_rejects_hash_mismatch_and_missing_artifact(tmp_path):
    intake = ExternalResearchIntake(tmp_path / "intake")
    bad = intake.register(source_id="SRC-BAD", title="bad", uri="https://example.invalid/bad.cif", local_path=FIXTURE, expected_sha256="0" * 64)
    missing = intake.register(source_id="SRC-MISSING", title="missing", uri="https://example.invalid/missing.cif", local_path=tmp_path / "missing.cif")
...
def test_external_intake_preserves_remote_unavailability_and_comparability(tmp_path):
    intake = ExternalResearchIntake(tmp_path / "intake")
    intake.register(source_id="SRC-REMOTE", title="remote", uri="https://example.invalid/source")
...
def test_external_intake_marks_identity_change_without_overwriting_registry(tmp_path):
    intake = ExternalResearchIntake(tmp_path / "intake")
    intake.register(source_id="SRC-CHANGE", title="source", uri="https://example.invalid/source")
    changed = intake.register(source_id="SRC-CHANGE", title="source changed", uri="https://example.invalid/source")
