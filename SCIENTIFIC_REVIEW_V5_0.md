...
Top-level Attempt 1 is a separate real operational record at `.research-os-live-5.0-top-level/`, and Attempt 2 is preserved at `.research-os-live-5.0-top-level-attempt-2/`. Attempt 2 reached `TL-CONSISTENCY-02-B` before a schema-valid response introduced the unregistered literal `CH-V45-SOLUBILITY-EXTERNAL-BOUNDARY`; its `UNKNOWN_GROUNDED_RECORD_ID` rejection is a correct fail-closed result and is classified as `MODEL_REFERENCE_HALLUCINATION`. The next acceptance freezes Run A's support basis for independent Run B and compares structured signatures; reviewer findings remain `REVIEW / ANALYSIS`, never Evidence.
