...
Top-level Attempt 1 is a separate real operational record at `.research-os-live-5.0-top-level/`; Attempts 2–5 are preserved in their numbered namespaces. Attempt 2 reached `TL-CONSISTENCY-02-B` before a schema-valid response introduced the unregistered literal `CH-V45-SOLUBILITY-EXTERNAL-BOUNDARY`; its `UNKNOWN_GROUNDED_RECORD_ID` rejection is a correct fail-closed result and is classified as `MODEL_REFERENCE_HALLUCINATION`. Attempts 3 and 4 reached `TL-CONSISTENCY-01-A`; their registered IDs passed general grounding, but the response omitted `primary_record_id` and `limitation_codes`, so Run B was not run. Attempt 4 established that only the outer envelope was machine-enforced. Attempt 5 reached the same call with the required fields still hidden in the payload rather than propagated as per-call transport context, so the transport selected the envelope and Run B was again withheld. The provider correction now forwards an allowlisted ephemeral context while preserving global security fields; the next acceptance binds the strict six-field schema at the transport boundary, freezes Run A's support basis for independent Run B, and compares structured signatures; reviewer findings remain `REVIEW / ANALYSIS`, never Evidence.
