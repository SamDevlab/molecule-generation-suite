...
Top-level Attempt 1 is a separate real operational record at `.research-os-live-5.0-top-level/`; Attempts 2–6 are preserved in their numbered namespaces. Attempt 2 reached `TL-CONSISTENCY-02-B` before a schema-valid response introduced the unregistered literal `CH-V45-SOLUBILITY-EXTERNAL-BOUNDARY`; its `UNKNOWN_GROUNDED_RECORD_ID` rejection is a correct fail-closed result and is classified as `MODEL_REFERENCE_HALLUCINATION`. Attempts 3 and 4 reached `TL-CONSISTENCY-01-A`; their registered IDs passed general grounding, but the response omitted `primary_record_id` and `limitation_codes`, so Run B was not run. Attempt 4 established that only the outer envelope was machine-enforced. Attempt 5 reached the same call with the required fields still hidden in the payload rather than propagated as per-call transport context, so the transport selected the envelope and Run B was again withheld. Attempt 6 reached the intended `CONSISTENCY`/`live_consistency.schema.json` route but stopped on a provider process error before schema validation. The provider correction now forwards an allowlisted ephemeral context while preserving global security fields; its provider-facing schema uses portable structural keywords and deterministic validators retain the semantic contract. The next action is the one-call schema smoke before a fresh Attempt 7. Reviewer findings remain `REVIEW / ANALYSIS`, never Evidence.
