...
# Configurações de controle de fluxo e ambiente
QUICK_TEST = False  # Mantido em True para validação e testes rápidos
...
logging.info(f"[DIRETÓRIO] Utilizando motor Vina local: {PATH_VINA}")
...
if QUICK_TEST:
    ...
    REPLICATAS = 1
    ESTIMADORES_RF = 10
    GRID_CONFIG = { "center_x": 24.0, "center_y": 24.0, "center_z": 48.0, "size_x": 10.0, "size_y": 10.0, "size_z": 10.0, "exhaustiveness": 1, "cpu": 1 }
    logging.info("[🚦] MODO QUICK_TEST ATIVO: Parâmetros reduzidos para feedback rápido.")
else:
    MAX_WORKERS = 4
    REPLICATAS = 3
    ESTIMADORES_RF = 100
    GRID_CONFIG = { "center_x": 24.0, "center_y": 24.0, "center_z": 48.0, "size_x": 20.0, "size_y": 20.0, "size_z": 20.0, "exhaustiveness": 8, "cpu": 1 }
    logging.info("[🏭] MODO DE PRODUÇÃO ATIVO: Executando triagem com exaustividade máxima.")

def run_vina(conf_file, out_pdbqt):
    try:
        if not os.path.exists(PATH_VINA):
            logging.error(f"[CRÍTICO] Executável do Vina não encontrado em: {PATH_VINA}")
            return None
...
# ==========================================================================
# MÓDULOS DE CIÊNCIA DE DADOS E GERAÇÃO REGULATÓRIA (HIGIENIZADO PARA LATIN-1)
# ==========================================================================
...
def gerar_graficos_e_pdf(df_hits):
    try:
        ...
        plt.title("Mapa de Sinergismo Termodinamico (Delta G kcal/mol)")
        plt.tight_layout()
        ...
        pdf.cell(200, 10, txt="Relatorio de Triagem Computacional G2-HPC", ln=True, align='C')
        pdf.set_font("Arial", size=10)
        pdf.cell(200, 10, txt="Validacao Estrutural, Lipinski e Docking Molecular", ln=True, align='C')
        pdf.ln(10)
        ...
        for _, row in df.head(5).iterrows():
            # HIGIENIZAÇÃO CRÍTICA: Substitui caracteres Unicode para evitar o erro de codec latin-1
            veredito_limpo = str(row['Veredito']).replace("★", "*").replace("⚠", "[!]")
            adme_limpo = str(row['Filtro_Lipinski_ADME'])
            
            linha = f"> {row['Par_Molecular']} | Sinergia C2: {row['Delta_G_Consenso_C2']} | ADME: {adme_limpo} | Veredito: {veredito_limpo}"
...
def treinar_modelo_ml():
    ...
    try:
        ...
        # Injeção de propriedades moleculares reais como Features para a IA aprender correlações físico-químicas
        X = df[['E_Isolada', 'LE_Isolado', 'MW_Complexo', 'LogP_Complexo', 'TPSA_Complexo']]
        y = df['Delta_G_Consenso_C2']
        
        modelo = RandomForestRegressor(n_estimators=ESTIMADORES_RF, random_state=42)
        modelo.fit(X, y)
        logging.info(f"[IA] Modelo de Machine Learning QSAR treinado com sucesso com {ESTIMADORES_RF} estimadores!")
    except Exception as e:
        logging.warning(f"Não foi possível treinar o modelo de IA nesta rodada: {e}")

# ==========================================================================
# MOTOR PRINCIPAL DE TRABALHO FARMACOTRÓPICO
# ==========================================================================
...
def buscar_e_filtrar_estruturas():
    if not os.path.exists(ARQUIVO_MATRIZ_LIMPA):
        # Fallback de contingência caso o coletor não tenha sido executado para o teste rápido
        logging.warning("Matriz limpa não encontrada! Gerando dados de contingência para teste...")
        with open(ARQUIVO_MATRIZ_LIMPA, mode='w', encoding='utf-8', newline='') as f:
            escritor = csv.writer(f)
            escritor.writerow(["Nome", "Smiles", "Classe"])
            escritor.writerow(["Celecoxib", "CC1=CC=C(C=C1)C2=CC(=NN2C3=CC=C(C=C3)S(=O)(=O)N)C(F)(F)F", "Medicamento"])
            escritor.writerow(["Piroxicam", "CN1C(=C(C2=CC=CC=C2S1(=O)=O)O)C(=O)NC3=CC=CC=N3", "Medicamento"])
            escritor.writerow(["Thymol", "CC1=CC(=C(C=C1)C(C)C)O", "Fitoterapico"])
            escritor.writerow(["Sulforaphane", "CS(=O)=CCCCN=C=S", "Fitoterapico"])
...
def baixar_e_preparar_receptores():
    ...
    for pdb_id in TODOS_ALVOS:
        ...
        if not os.path.exists(pdb_file):
            logging.info(f"Baixando estrutura experimental: {pdb_id}...")
...
# ==========================================================================
# CORAÇÃO CIENTÍFICO REAL: AVALIAÇÃO FILTRO DE LIPINSKI (ADME) VIA RDKIT
# ==========================================================================
def avaliar_lipinski_real(smiles_med, smiles_nat):
    try:
        mol_med = Chem.MolFromSmiles(smiles_med)
        mol_nat = Chem.MolFromSmiles(smiles_nat)
        
        # Extração de parâmetros físico-químicos empíricos reais de Lipinski
        mw_complexo = Descriptors.MolWt(mol_med) + Descriptors.MolWt(mol_nat)
        logP_complexo = Crippen.MolLogP(mol_med) + Crippen.MolLogP(mol_nat)
        h_donors = Descriptors.NumHDonors(mol_med) + Descriptors.NumHDonors(mol_nat)
        h_acceptors = Descriptors.NumHAcceptors(mol_med) + Descriptors.NumHAcceptors(mol_nat)
        tpsa_complexo = Descriptors.TPSA(mol_med) + Descriptors.TPSA(mol_nat)
        
        # Filtros rígidos da Regra dos Cinco de Lipinski aplicados ao complexo sinérgico
        violacoes = 0
        if mw_complexo > 600: violacoes += 1  # Limite estendido tolerante para a formulação combinada
        if logP_complexo > 6.0: violacoes += 1
        if h_donors > 6: violacoes += 1
        if h_acceptors > 12: violacoes += 1
        
        if violacoes <= 1:
            veredito_adme = "Aprovado Lipinski"
        else:
            veredito_adme = "Alerta ADME Restrito"
            
        return veredito_adme, mw_complexo, logP_complexo, tpsa_complexo
    except:
        return "Erro Químico", 0.0, 0.0, 0.0
...
def processar_par_industrial(med, nat, id_par):
    ...
    # Executa o filtro de Lipinski com base nos grafos químicos reais
    adme_status, complex_mw, complex_logp, complex_tpsa = avaliar_lipinski_real(med_smiles, nat_smiles)
    ...
    try:
        ...
        # Aplicação cruzada de critérios de elegibilidade farmacêutica reais
        if c2_delta <= -0.5 and seletividade > 0.1 and adme_status == "Aprovado Lipinski":
            veredito = "HIT REFINADO ★"
        elif c2_delta <= -0.5 and adme_status == "Alerta ADME Restrito":
            veredito = "HIT C/ RESSALVA ⚠"
        else:
            veredito = "NEUTRO"
        
        alerta_tox = "N/A"
        if veredito in ["HIT REFINADO ★", "HIT C/ RESSALVA ⚠"]:
        ...
        # Retorno da linha com os novos descritores empíricos reais integrados para calibração da IA
        return [
            nome_par, f"{c2_iso:.2f}", f"{c2_sin:.2f}", f"{c2_delta:.2f}", f"{c2_dp:.3f}", f"{seletividade:.2f}", 
            f"{le_iso:.3f}", f"{delta_le:.3f}", f"{complex_mw:.2f}", f"{complex_logp:.2f}", f"{complex_tpsa:.2f}",
            adme_status, f"{calcular_ki(c2_iso):.2f}", f"{calcular_ki(c2_sin):.2f}", alerta_tox, veredito
        ]
    except Exception as e:
        return None
...
def processar_relatorio_top_hits():
    ...
    # Ordenação estrita pelo delta de energia e índice de seletividade
    ...
    try:
        ...
        if df_validacao.empty or len(df_validacao) == 0:
            raise ValueError("Arquivo temporário gerado vazio. Abortando substituição.")
        ...
        logging.error(f"SISTEMA DE SALVAGUARDA: Falha ao gerar ranking: {e}")
        if os.path.exists(arquivo_temp): os.remove(arquivo_temp)

def main():
    logging.info("==========================================================================")
    logging.info("   PLANTA G2-HPC v2.1: TRIAGEM COM VALIDAÇÃO REAL ADME (LIPINSKI)         ")
    logging.info("==========================================================================")
    ...
    if os.path.exists(ARQUIVO_BRUTO):
        ...
        with open(ARQUIVO_BRUTO, mode='w', encoding='utf-8', newline='') as f:
            # Cabeçalho expandido contendo as colunas dos descritores moleculares estruturais reais
            csv.writer(f).writerow([
                "Par_Molecular", "E_Isolada", "E_Sinergica", "Delta_G_Consenso_C2", "Desvio_Padrão_C2", "Índice_Seletividade", 
                "LE_Isolado", "Delta_LE", "MW_Complexo", "LogP_Complexo", "TPSA_Complexo", "Filtro_Lipinski_ADME",
                "Ki_Isolado_uM", "Ki_Sinergico_uM", "Docking_Reverso_Tox", "Veredito"
            ])
    ...
    if not fila_producao:
        logging.info("[✓] Todos os pares combinatórios já calculados!")
        processar_relatorio_top_hits()
        return

    # Se o QUICK_TEST estiver ativo, limita a fila de processamento a 4 amostras para feedback instantâneo
    if QUICK_TEST and len(fila_producao) > 4:
        logging.info(f"QUICK_TEST ativo: Limitando fila de {len(fila_producao)} -> 4 amostras para execução rápida.")
        fila_producao = fila_producao[:4]

    logging.info(f"Disparando HPC por CPU ({MAX_WORKERS} Núcleos) | {len(fila_producao)} Rotas na Fila...")
