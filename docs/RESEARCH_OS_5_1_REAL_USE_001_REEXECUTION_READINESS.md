...
|---|---|---|
| BRK-001 | canonical source registration, provenance, fingerprint and idempotence | Implemented and unit-tested |
| BRK-002 | deterministic Gemmi mmCIF parser, canonical normalization, fail-closed ligand identity | Implemented and unit-tested with 5KIR/5IKR fixtures |
| BRK-003 | deterministic atom mapping, declared symmetry handling, RMSD diagnostics | Implemented and unit-tested |
| BRK-004 | formal Vina/Open Babel adapter registration and preflight | Implemented; current development host reports both external executables unavailable |
...
`EnginePreflight` must return `READY` for both `autodock-vina` and `openbabel`
